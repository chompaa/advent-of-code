...
_lines = list(filter(lambda x: x[0][0] == x[1][0] or x[0][1] == x[1][1], lines))
...
# remove x1 == x2 and y1 == y2 entries
_lines = list(filter(lambda x: x[0][0] != x[1][0] and x[0][1] != x[1][1], lines))

# remove non 45 degree angle entries
_lines = list(filter(lambda x: abs((x[0][1] - x[1][1])/(x[0][0] - x[1][0])) == 1, _lines))
